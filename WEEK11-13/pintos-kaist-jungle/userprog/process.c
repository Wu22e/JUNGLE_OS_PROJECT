...
__do_fork(void *aux) {
    ...
    struct intr_frame *parent_if_ = (struct intr_frame *)aux;
    bool succ = true;
    ...
    for (int i = 2; i < parent->max_fd; i++) {
        if (parent->fd_table[i] != NULL) {
            thread_current()->fd_table[i] = file_duplicate(parent->fd_table[i]);
        }
    }

    thread_current()->max_fd = parent->max_fd;
    // thread_current()->current_file = parent ->current_file;
    process_init();
    sema_up(&parent->fork_semaphore);
    ...
    do_iret(&if_);
error:
    thread_current()->child_fork_status = -1;
    sema_up(&parent->fork_semaphore);
    thread_exit();
}
...
int process_exec(void *f_name) {
    ...
    */
    ...
    // load를 통해 _if.rsp가 유저영역의 맨 윗부분인 stack(비어있음)의 top을 가리키게 된다
    // 오직 fork를 통해 exec 된
    ...
    // printf("------after  arg-stack:     %p -------\n", _if.rsp);
    // if load failed, quit
    // palloc_free_page(file_name); // 위치확인

    if (!success) {
        palloc_free_page(file_name);  // 여긴 어떄?
        return -1;
    }
...
load(const char *file_name, struct intr_frame *if_) {
    ...
    // file_close (file);
    // file_deny_write(file);
    // thread_current()->current_file = NULL;
    return success;
}
...
void argument_stack(char **parse, int count, void **rsp, struct gp_registers *R) {
    int i, j;
    int len = 0;
    ...
    *rsp = *rsp - 8;
    ...
    uint64_t *argv_start = *rsp;
    *rsp = *rsp - 8;
    **(uint64_t **)rsp = argv_start;
...
void *remove_child_process(struct thread *cp) {
    ...
    // list_remove(&cp->elem); // destruction에 들어가있던애
