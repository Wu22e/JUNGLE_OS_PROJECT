...
static struct list ready_list; // CPU를 기다리는 쓰레드의 배열


static struct list sleep_list; /* THREAD_BLOCKED 상태의 스레드를 관리하기 위한 리스트 자료 구조 추가 */

static int64_t next_tick_to_awake; /* sleep_list 에서 대기중인 스레드들의 wakeup_tick 값 중 최소값을 저장하기 위한 변수 추가 */
...
thread_init (void) {
	...
	list_init (&sleep_list); /* sleep_list 를 초기화 */
...
/* 실행 중인 스레드를 슬립으로 만듦, 인자로 들어온 ticks까지 재움 */
void thread_sleep(int64_t ticks) {
	...
	//printf("before block thread_sleep is called !!! \n");

	//printf("after block thread_sleep is called !!! \n");

	update_next_tick_to_awake(curr->wakeup_tick = ticks); // 현재의 thread의 wakeup_ticks에 인자로 들어온 ticks를 저장후 next_tick_to_awake를 업데이트
	list_push_back(&sleep_list, &(curr->elem)); // sleep_list에 현재 thread의 element를 슬립 리스트의 마지막에 삽입
	
	thread_block(); // 현재 thread를 block 시킴

	/* 현재 스레드를 슬립 큐에 삽입한 후에 스케줄 한다. 
	   해당 과정중에는 인터럽트를 받아들이지 않는다. */

	intr_set_level(old_level); // 다시 스케줄하러 갈 수 있게 만들어줌(인터럽트가 가능하도록 함)
}

/* 슬립 큐에서 깨워야 할 스레드를 깨움 */
